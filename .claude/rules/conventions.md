...
- Nomenclatura para endpoints com nomes compostos deve-se utilizar sempre `-`. Por exemplo: `base_url/v1/my-fun-model/my-cool-endpoint`.
...
- Cada teste individual deve conter seu próprio banco de dados temporário, ou seja, se no teste A foi criado um usuário, o teste B não o verá. Caso o teste B precise de um usuário, ele deve criar novamente este usuário no seu próprio banco.
    ...
    - **"A execução" é uma invocação de `go test`, não a suíte conceitual.** O reaper destrói o container quando a invocação que o criou termina (medido: some em menos de 5s). Por isso a suíte completa tem que rodar numa invocação só (`task test-all` = `go test ./tests/... -v`), igual ao CI. Encadear invocações entrega à fase seguinte um container já sentenciado: ela se anexa por nome, recebe um DSN válido, começa a rodar, e então o reaper a derruba no meio — todos os testes restantes daquela fase morrem com `failed to receive message: unexpected EOF`, que parece teste quebrado e não é. Foi assim que a 0.46.2 nasceu.
